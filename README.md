...
A small Go CLI for managing **git notes** as reviewable, CSV-backed comments on `HEAD` — and posting them as GitHub PR / GitLab MR review comments.
...
Posting to a PR/MR additionally needs the [`gh`](https://cli.github.com/) (GitHub) or [`glab`](https://gitlab.com/gitlab-org/cli) (GitLab) CLI, installed and authenticated.
...
`submit` requires the **PR/MR number** and derives the **diff base** from it — GitHub's base branch (e.g. `origin/main`), GitLab's `diff_refs.base_sha` — so there is no `-b` flag. It then classifies each note against that diff:
